...
function Shop2() {
  ...
  const {blazer ,blazerItems , selectedBrands , handleChange , sortingData ,handleChangeAvailable, notAvailableItems , availableItems , selectedAvailable } = useContext(blazerContext)
  
  const products = blazerItems.map((el , index)=> {
    return (
  ...
  const uniqueAvailable = [...new Set(blazer.map(item => item.available))];
  ...
  const availableCasual = uniqueAvailable.map((el, index) => {
    ...
    const uniqueBrands = [...new Set(blazer.map(item => item.brand))];
