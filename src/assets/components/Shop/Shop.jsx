...
function Shop() {
  ...
  const {casualItems , handleChange ,casual , selectedAvailable , selectedBrands  , availableItems , handleChangeAvailable , notAvailableItems , sortingData} = useContext(casualContext)
  
  const products = casualItems.map((el , index)=> {
    return (
 ...
 // get unique available
  const uniqueAvailable = [...new Set(casual.map(item => item.available))];
  const availableCasual = uniqueAvailable.map((el, index) => {
    if (el == true) {
      return (
    ...
    // // get unique color
    // const uniqueColor = [...new Set(casual.flatMap(item => item.color))]

    // const colorCasual = [...new Set(uniqueColor?.map(item => item.color))];

    // console.log(colorCasual)
    ...
    <div className="Shop SecThree">
      <div className="title">
        <div className="container  py-5">
          ...
          <p className="h2 mt-4">Casual Wear</p>
        </div>
      </div>
      <div className="body">
          ...
          <div className="row g-0">
            ...
            <div className="col-lg-9 col-md-8 col-sm-12">
              ...
              <div className="d-flex justify-content-between align-items-center px-3 py-2" style={{ fontSize: "12px",borderBottom : "1px solid white"}}>
                <p className="m-0">Home / <span>Casual Wear</span></p>
