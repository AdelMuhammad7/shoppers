...
function Shop3() {
  ...
  const {trouser , trouserItems , selectedBrands , handleChange ,sortingData , availableItems, notAvailableItems, handleChangeAvailable ,selectedAvailable} = useContext(trouserContext)
  
  const products = trouserItems.map((el , index)=> {
    return (
  ...
  const uniqueAvailable = [...new Set(trouser.map(item => item.available))];
  ...
  const availableCasual = uniqueAvailable.map((el, index) => {
    ...
    const uniqueBrands = [...new Set(trouser.map(item => item.brand))];
...
{/* 
                <div className="d-flex justify-content-between align-items-center" style={{fontSize : "12px"}}>
                    <p><span>{trouser ? trouser.length : 0}</span> selected</p>
            ...
            <div className="col-lg-9 col-md-8 col-sm-12">
              ...
              <div className="d-flex justify-content-between align-items-center px-3 py-2" style={{ fontSize: "12px",borderBottom : "1px solid white"}}>
                <p className="m-0">Home / <span>trouser</span></p>
